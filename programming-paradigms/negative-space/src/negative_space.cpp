...
public:
    ...
    void setValue(const std::string& value) {
        ...
        if (value.length() > m_maxLength) {
            // make this info since we have negative tests, but should be error/critical
            LOG_INFO(std::format("String exceeds maximum length of {}", m_maxLength));
        ...
        for (const char c : value) {
            if (std::find(m_forbiddenChars.begin(), m_forbiddenChars.end(), c) != m_forbiddenChars.end()) {
                // make this info since we have negative tests, but should be error/critical
                LOG_INFO(std::format("String contains forbidden character: {}", static_cast<int>(c)));
        ...
        m_value = value;
        LOG_INFO(std::format("Successfully set string value: {}", m_value));
    }
    ...
    void addForbiddenChar(const char bad_char) {
        ...
        if (std::find(m_forbiddenChars.begin(), m_forbiddenChars.end(), bad_char) != m_forbiddenChars.end()) {
            LOG_ERROR(std::format("Character {} is already forbidden", static_cast<int>(bad_char)));
            return;
        }

        m_forbiddenChars.push_back(bad_char);
        LOG_INFO(std::format("Added forbidden character: {}", static_cast<int>(bad_char)));

        // if the current value contains the newly forbidden character, clear it
        if (m_value.find(bad_char) != std::string::npos) {
            m_value.clear();
            LOG_INFO("Cleared current value due to new forbidden character");
        }
    }
...
int main() {
    ...
    try {
        ...
        LOG_ERROR(std::format("Test case 1 failed: {}", e.what()));
        return 1;
    }
    ...
    try {
        SafeString ss;
        ss.setValue("Hello\nWorld");
        LOG_ERROR("Test case 3 failed: Control character was accepted");
        return 1;
    } catch (const std::invalid_argument&) {
        LOG_INFO("Test case 3 passed: Control character rejected");
    }
    ...
    try {
        ...
        LOG_ERROR("Test case 4 failed: Oversized string was accepted");
        return 1;
    } catch (const std::invalid_argument&) {
    ...
    try {
        ...
        LOG_INFO("Test case 5 passed: String cleared after adding forbidden character");
    } catch (const std::exception& e) {
        LOG_ERROR(std::format("Test case 5 failed: {}", e.what()));
        return 1;
    }
