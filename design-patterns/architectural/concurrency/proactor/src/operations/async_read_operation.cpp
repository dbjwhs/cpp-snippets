...
bool AsyncReadOperation::initiate(const std::shared_ptr<Proactor>& proactor) {
    Logger::getInstance().log(LogLevel::INFO, 
        std::format("Initiating read operation on socket {}", m_socket));
    ...
    if (getsockopt(m_socket, SOL_SOCKET, SO_ERROR, &error, &errLen) < 0) {
        Logger::getInstance().log(LogLevel::ERROR, 
            std::format("Socket diagnostic failed: {}", strerror(errno)));
        return false;
    }
    
    if (error != 0) {
        Logger::getInstance().log(LogLevel::ERROR, 
            std::format("Socket has error state: {}", strerror(error)));
        return false;
    }
    ...
    if (const ssize_t result = recv(m_socket, testBuf, 0, MSG_PEEK); result < 0 && errno != EAGAIN && errno != EWOULDBLOCK) {
        Logger::getInstance().log(LogLevel::ERROR, 
            std::format("Socket read test failed: {}", strerror(errno)));
        return false;
    }
