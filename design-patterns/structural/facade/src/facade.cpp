...
public:
    bool initialize() {
        ...
        if (!detectAudioDevice()) {
            ...
            Logger::getInstance().log(LogLevel::ERROR, std::format("failed to detect audio device"));
            return false;
        }
        ...
        if (!initializeAudioBuffers()) {
            ...
            Logger::getInstance().log(LogLevel::ERROR, std::format("failed to initialize audio buffers"));
            return false;
        }
        ...
        if (!checkAudioPermissions()) {
            ...
            Logger::getInstance().log(LogLevel::ERROR, std::format("insufficient audio permissions"));
            return false;
        }

        m_initialized = true;
        Logger::getInstance().log(LogLevel::INFO, std::format("audio system initialized successfully"));
        return m_initialized;
    }

    void setVolume(float volume) {
        if (!m_initialized) {
            Logger::getInstance().log(LogLevel::ERROR, std::format("cannot set volume - system not initialized"));
            return;
        }
...
public:
    bool initialize() {
        ...
        if (!detectDisplayDevice()) {
            m_lastError = std::make_error_code(std::errc::no_such_device);
            Logger::getInstance().log(LogLevel::ERROR, std::format("no compatible display device found"));
            return false;
        }
        ...
        if (!checkResolutionSupport(m_width, m_height)) {
            ...
            Logger::getInstance().log(LogLevel::ERROR, std::format("display does not support required resolution"));
            return false;
        }
        ...
        if (!initializeFrameBuffer()) {
            m_lastError = std::make_error_code(std::errc::not_enough_memory);
            Logger::getInstance().log(LogLevel::ERROR, std::format("failed to initialize frame buffer"));
            return false;
        }

        m_initialized = true;
        Logger::getInstance().log(LogLevel::INFO, std::format("video system initialized with resolution {}x{}", m_width, m_height));
        return true;
    }

    void setResolution(uint32_t width, uint32_t height) {
        if (!m_initialized) {
            Logger::getInstance().log(LogLevel::ERROR, std::format("cannot set resolution - system not initialized"));
            return;
        }
        if (!checkResolutionSupport(width, height)) {
            Logger::getInstance().log(LogLevel::ERROR, std::format("requested resolution {}x{} not supported", width, height));
            return;
        }
        m_width = width;
        m_height = height;
        Logger::getInstance().log(LogLevel::INFO, std::format("resolution changed to {}x{}", m_width, m_height));
    }
...
public:
    bool initialize() {
        ...
        if (!detectInputDevices()) {
            m_lastError = std::make_error_code(std::errc::no_such_device);
            Logger::getInstance().log(LogLevel::ERROR, std::format("no input devices detected"));
            return false;
        }
        ...
        if (!initializeInputBuffers()) {
            m_lastError = std::make_error_code(std::errc::not_enough_memory);
            Logger::getInstance().log(LogLevel::ERROR, std::format("failed to initialize input buffers"));
            return false;
        }
        ...
        if (!registerInputHandlers()) {
            ...
            Logger::getInstance().log(LogLevel::ERROR, std::format("failed to register input handlers"));
            return false;
        }

        m_initialized = true;
        Logger::getInstance().log(LogLevel::INFO, std::format("input system initialized successfully"));
        return true;
    }

    void setMouseEnabled(bool enabled) {
        if (!m_initialized) {
            Logger::getInstance().log(LogLevel::ERROR, std::format("cannot set mouse state - system not initialized"));
            return;
        }
...
public:
    ...
    bool initialize() {
        Logger::getInstance().log(LogLevel::INFO, std::format("initializing game system facade"));

        // initialize video first as other systems may depend on it
        if (!m_video->initialize()) {
            m_lastError = m_video->getLastError();
            Logger::getInstance().log(LogLevel::ERROR, std::format("video system initialization failed"));
            return false;
        }
        ...
        if (!m_audio->initialize()) {
            ...
            Logger::getInstance().log(LogLevel::ERROR, std::format("audio system initialization failed"));
            return false;
        }
        ...
        if (!m_input->initialize()) {
            ...
            Logger::getInstance().log(LogLevel::ERROR, std::format("input system initialization failed"));
            return false;
        }

        m_initialized = true;
        Logger::getInstance().log(LogLevel::INFO, std::format("game system facade initialization complete"));
        return true;
    }
    ...
    bool configureDefaultGameSettings() {
        ...
        Logger::getInstance().log(LogLevel::INFO, std::format("default game settings configured"));
        return true;
    }
...
int main() {
    ...
    Logger::getInstance().log(LogLevel::INFO, std::format("test 2: initialization successful"));
    ...
    Logger::getInstance().log(LogLevel::INFO, std::format("test 4: default settings verified"));

    // test complete
    Logger::getInstance().log(LogLevel::INFO, std::format("all facade pattern tests completed successfully"));
    return 0;
}
